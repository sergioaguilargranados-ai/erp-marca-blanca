import { db } from '@/lib/db'
import { empresas, categorias } from '@/lib/db/schema'
import { eq } from 'drizzle-orm'
import { redirect } from 'next/navigation'

export default async function NuevaCategoriaPage({ params }: { params: { id: string } }) {
  const [empresa] = await db
    .select()
    .from(empresas)
    .where(eq(empresas.id, params.id))
    .limit(1)

  if (!empresa) {
    redirect('/admin/empresas')
  }

  // Obtener categorías existentes para jerarquía
  const categoriasExistentes = await db
    .select()
    .from(categorias)
    .where(eq(categorias.empresaId, params.id))
    .orderBy(categorias.nombre)

  const crear = async (formData: FormData) => {
    'use server'

    const nombre = formData.get('nombre') as string
    const descripcion = formData.get('descripcion') as string
    const codigo = formData.get('codigo') as string
    const categoriaPadre = formData.get('categoriaPadre') as string

    await db.insert(categorias).values({
      empresaId: params.id,
      nombre,
      descripcion: descripcion || null,
      codigo: codigo || null,
      categoriaPadre: categoriaPadre || null,
      activa: true,
    })

    redirect(`/admin/empresas/${params.id}/categorias?success=categoria-creada`)
  }

  return (
    <div className="max-w-3xl mx-auto">
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-8">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Nueva Categoría</h1>
            <p className="text-slate-600 mt-1">Para {empresa.nombre}</p>
          </div>
          <a
            href={`/admin/empresas/${params.id}/categorias`}
            className="text-slate-600 hover:text-slate-900"
          >
            ← Volver
          </a>
        </div>

        <form action={crear} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              Nombre de la Categoría *
            </label>
            <input
              type="text"
              name="nombre"
              required
              className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
              placeholder="Electrónica, Ropa, Alimentos..."
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              Código (opcional)
            </label>
            <input
              type="text"
              name="codigo"
              className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
              placeholder="CAT-001"
            />
            <p className="text-xs text-slate-500 mt-1">
              Código interno para identificar la categoría
            </p>
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              Descripción
            </label>
            <textarea
              name="descripcion"
              rows={3}
              className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
              placeholder="Descripción de la categoría..."
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              Categoría Padre (opcional)
            </label>
            <select
              name="categoriaPadre"
              className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
            >
              <option value="">Sin categoría padre (categoría principal)</option>
              {categoriasExistentes.map((cat) => (
                <option key={cat.id} value={cat.id}>
                  {cat.nombre}
                </option>
              ))}
            </select>
            <p className="text-xs text-slate-500 mt-1">
              Crea una jerarquía de categorías (ej: Electrónica → Computadoras)
            </p>
          </div>

          <div className="flex gap-3 pt-4 border-t border-slate-200">
            <button
              type="submit"
              className="flex-1 bg-emerald-600 text-white py-3 rounded-lg font-medium hover:bg-emerald-700 transition-colors"
            >
              Crear Categoría
            </button>
            <a
              href={`/admin/empresas/${params.id}/categorias`}
              className="flex-1 text-center bg-slate-200 text-slate-700 py-3 rounded-lg font-medium hover:bg-slate-300 transition-colors"
            >
              Cancelar
            </a>
          </div>
        </form>
      </div>
    </div>
  )
}
